...
def format_forecast_message(forecast_data: dict) -> str:
    ...
    for i, day in enumerate(forecast_data['forecasts']):
        ...
        message += f"📌 *{header}* {day_date.strftime('%d.%m')}\n"
...
async def about_bot(message: Message):
    about = (
        ...
        "✨ *Функции:*\n"
        "• Текущая погода\n"
        "• Прогноз на 5 дней с русскими днями недели\n"
...
def send_daily_weather():
    ...
    weather = get_5day_forecast(city)
    asyncio.create_task(bot.send_message(chat_id=chat_id, text=format_forecast_message(weather), parse_mode="Markdown"))
...
async def main():
    ...
    print("🌐 Дни недели в прогнозе: РУССКИЕ")
    print("="*60 + "\n")
